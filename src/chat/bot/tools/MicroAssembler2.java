 ...
 * @version 3.7
 */
public final class MicroAssembler2{
	private MicroAssembler2(){}
	/**The registers.*/
			...
			switch(kind){
				...
				case ParType.AlfaAlfanum:
					if(args.matches(rRegx+rRegx))
						return new Object[]{args.charAt(0), MicroAssembler2.this.registers[correct(args.charAt(1))]};
			...
			void action(Object[] args){
				...
				if(txt.matches(contregx)){
					...
					while(txt.matches(contregx)){
						...
						txt = txt.replace(rvalch+""+ch, ""+MicroAssembler2.this.registers[correct(ch)]);
					}
				}
				output+=txt;
			}
	...
	public static synchronized String assemble(String input){
		...
		return new MicroAssembler2().assemble(input, arguments);
	}
	 ...
	 */
	public synchronized String assemble(String input, String arguments){
